    ...
    return (
        <div ref={heroRef} className="w-[100%] flex flex-col items-center justify-center">
            <div className="w-[100%] mx-auto md:w-[902px] flex flex-col gap-[16px] md:gap-[32px] text-center items-center justify-center">
                <p className={`text-[#0e0e0e] text-[34px] md:text-[64px] leading-[40px] md:leading-[72px] font-[800] transition-all duration-600 ease-out ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`} style={{ transitionDelay: isVisible ? '100ms' : '0ms' }}>Design assets that ship fast <span className="text-[#dbdbdb]">Icons, Illustrations, Emojis.</span></p>
                <p className={`text-[#B7B7B7] text-[12px] md:text-[16px] leading-[20px] md:leading-[24px] font-[400] transition-all duration-600 ease-out ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`} style={{ transitionDelay: isVisible ? '200ms' : '0ms' }}>Pixel‑perfect SVGs and vectors for web, iOS, and Android — free to start, pro when you scale.</p>
            </div>
            <div className="mx-auto flex justify-center justify-center w-[100%]">
                <div className="w-[100%] grid grid-cols-2 md:grid-cols-4 md:gap-0 gap-[32px] mt-[56px] mx-auto">
                    <div className={`flex flex-col gap-[8px] justify-start items-center text-center transition-all duration-600 ease-out ${
                        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
