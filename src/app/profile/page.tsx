...
export default function Profile() {
    ...
    useEffect(() => {
        const refreshUserData = async () => {
            ...
            if (token && user) {
                try {
                    const response = await fetch('https://figcons.vercel.app/api/auth/profile', {
                        headers: {
    ...
    useEffect(() => {
        const handlePaymentCallback = async () => {
            ...
            if (paymentStatus === 'success' && sessionId) {
                ...
                if (token) {
                    try {
                        const response = await fetch('https://figcons.vercel.app/api/payment/confirm-checkout-session', {
                            method: 'POST',
                            headers: {
    ...
    const handleProUpgrade = async () => {
        ...
        try {
            ...
            const response = await fetch('https://figcons.vercel.app/api/payment/create-checkout-session', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                        ...
                        <div className="flex flex-col gap-[16px]">
                            <div className="p-[24px] bg-gradient-to-r from-[#0E0E0E] to-[#333333] rounded-[16px] text-white">
                                ...
                                <button
                                    ...
                                    className={`w-full h-[48px] rounded-[12px] font-bold text-[14px] transition-all duration-200 flex items-center justify-center gap-[8px] ${
                                        isProcessingPayment
