...
export default function IconsEditor({ selectedIcon }: IconsEditorProps) {
    ...
    const tags = [
        "Fingerprint", "Thumbprint", "Authentication", "Scan", "Touch ID",
        "Secure", "Security", "Biometric", "Identity", "Scanner",
        "Login", "Password", "Identification", "Access", "Touch", "ID",
        "Lock", "Interface", "Box", "Frame",
    ];

    return (
        <div className="w-full md:w-[252px] 2xl:w-[300px] 3xl:w-[350px] h-full flex flex-col gap-[16px] md:px-0 px-[16px] py-[16px] md:py-0 md:pb-[8px]">
            ...
            <div className="flex flex-col gap-[12px]">
                ...
                <div className="border border-[#ececec] rounded-[20px] h-[213px] 2xl:h-[250px] 3xl:h-[280px] flex justify-center items-center">
                    <Svg
                        w={`${Math.min(size, 190)}px`}
                        h={`${Math.min(size, 190)}px`}
                        icon={selectedIcon as keyof typeof import("@/public/assets").assets}
                    />
                </div>

                <div className="flex gap-[8px] items-center">
                    <button className="px-[12px] py-[8px] w-[50%] rounded-full bg-[#0e0e0e] text-[#ffffff] text-[12px] font-bold leading-[20px] h-[40px] 2xl:h-[48px] 3xl:h-[52px]">
                        Copy
                    </button>
                    <button className="px-[12px] py-[8px] w-[50%] rounded-full bg-[#f6f6f6] text-[#0e0e0e] text-[12px] font-bold leading-[20px] h-[40px] 2xl:h-[48px] 3xl:h-[52px]">
                        Download
                    </button>
                </div>
            </div>
